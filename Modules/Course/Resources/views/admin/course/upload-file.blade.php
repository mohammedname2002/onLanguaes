...
@section('title','رفع ملف للكورس')

@section('title_page','رفع ملف للكورس')
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0">رفع ملف للكورس</h4>

                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="javascript: void(0);">الصفحة الرئيسية</a></li>
                            <li class="breadcrumb-item active">رفع ملف للكورس</li>
                        </ol>
                    </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div id="upload-container" class="text-center">
                            <button id="browseFile" class="btn btn-primary">الرجاء الضغط لإختيار ملف</button>
                        </div>
                        <div  style="display: none" id="uploaddiv" class="mt-3 text-center" style="height: 24px !important;">
                            <div class="form-group">
                                <label class="form-label" class="text-right">وصف الملف بالعربي</label>
                                 <textarea name="description_ar" id="file_description_ar" cols="30" rows="10" class="form-control"></textarea>
                            </div>
                            <div class="form-group">
                                <label class="form-label" class="text-right">وصف الملف بالإنجليزي</label>
                                 <textarea name="description_en" id="file_description_en" cols="30" rows="10" class="form-control"></textarea>
                            </div>
                            <button id="uploadfile" onclick="uploadFile()" class="btn btn-success mt-5">رفع الملف</button>
                        </div>
            ...
            <div class="col-xl-12 col-xxl-12 col-lg-12 col-md-12 col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">مرفق الدورة</h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table verticle-middle table-responsive-md">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$course->attachment->title}}</td>
                                        <td>
                                            @if ($course->attachment->type=="image")
                                            {{"صورة"}}
                                              @else
                                               {{"ملف"}}
                                        ...
                                        <td>
                                            <div class="row text-center">
                                                <div class="col-lg-4">
                                                    <a href="{{route('admin.course.attachment.download',[$course->id,$course->attachment->id])}}" class="btn btn-success"><i class="fa fa-download" aria-hidden="true"></i></a>

                                                </div>
                                                <div class="col-lg-4">
                                                    <form action="{{route('admin.course.attachment.delete',[$course->id,$course->attachment->id])}}" method="post">
                                                        @csrf
...
<script type="text/javascript">
    ...
    let resumable = new Resumable({
        target: '{{ route('admin.course.upload',$course->id) }}',
    ...
    resumable.on('fileSuccess', function (file, response) { // trigger when file upload complete
        ...
        $('#message').append("<a class='btn btn-sucesss' href='{{route('admin.course.index')}}'>الرجوع للخلف</a>");
        $('.card-footer').show();
        hideProgress();
