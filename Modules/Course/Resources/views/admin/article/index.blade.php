                        ...
                        <h4 class="card-title">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-6" style="text-align: left">
                                    <a href="{{route('admin.article.create')}}" class="btn btn-primary">إنشاء</a>
                                </div>
                            </div>
                        </h4>
                                 ...
                                 <form action="" class="row">
                                    <div class="col-sm-12 col-md-6" >
                                        <div id="datatable_filter" class="dataTables_filter" style="text-align: right !important">
                                            ...
                                            <span style="margin-right: 8px;">عدد المقالات:{{$info->count}}</span>
                            ...
                            <table style="border-collapse: collapse; border-spacing: 0px; width: 100%;" id="datatable" class="table table-bordered dt-responsive nowrap dataTable no-footer dtr-inline">
                                ...
                                <tbody>
                                    ...
                                    <tr data-id="1" style="cursor: pointer;">
                                        <td data-field="id" style="width: 80px">{{ $articles->firstItem() + $loop->index }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $article->title_ar }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $article->title_en }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $article->attachments }}</td>
                                        <td data-field="age" style="width: 182.783px;">{{ $article->created_at->format('y-m-d') }}</td>
                                        <td style="width: 200px;" class="d-flex justify-content-center">
                                            @if ($admin->can('تعديل مقال') || $admin->can('جميع الصلاحيات super_admin')  )

                                            <a class="btn btn-outline-secondary btn-sm edit" href="{{route('admin.article.edit',$article->id)}}" title="تعديل">
                                                <i class="fas fa-pencil-alt" title="تعديل"></i>
                                            </a>
                                            @endif
                                            @if ($admin->can('عرض ملف مقال') || $admin->can('جميع الصلاحيات super_admin')  )
                                            <a class="btn btn-outline-secondary btn-sm edit text-success" href="{{route('admin.article.upload.page',$article->id)}}" style="margin:0 4px;" title="رفع ملف">
                                                <i class="fas fa-upload" title="رفع ملف"></i>
                                            </a>
                                            @endif
                                            @if ($admin->can('حذف مقال') || $admin->can('جميع الصلاحيات super_admin')  )
                                             <form action="{{route('admin.article.delete',$article->id)}}" id="deleteform{{$article->id}}" method="POST" style="margin-right:4px;">
                                                @csrf
                                                <button type="submit" onclick="JSconfirm(event,{{$article->id}})" class="btn btn-outline-secondary btn-sm edit">
