...
<main style="margin-top: 100px">
    ...
    <div class="video-main-content container-fluid">
      ...
      <div class="content-one-video" style="min-width: 60%">
        ...
        <smartvideo poster=" {{ asset($PaidDetails->poster) }}" class="swarm-fluid aa" controls playsinline

            src="{{ $PaidDetails->path }}">
           </smartvideo>


           @if (App::getLocale() == 'en')
           <h3>   {{  $PaidDetails->title_en }}  </h3>
                @else
                <h3>  {{  $PaidDetails->title_ar }}  </h3>
                @endif
...
<div style="position: relative;margin: 0px 0;padding: 11px 0;">
    <div class="reactions" style="display: flex;margin-top:0 !important;margin-left:5px !imporatnt;flex-direction: row !important;top:0 !important">          
      ...
      <button  onclick="likedVideos()"   class="like" >
        ...
        <i class="fas fa-heart heart" style="color:{{ $PaidDetails->liked()?'red':'' }}" id="button1"></i>
     </button>
 ...
 <div class="teacher-info">
            ...
            <div class="description">
               @if(App::getLocale() == 'ar')
                وصف الدورة
                <p> {!! $PaidDetails->description_ar !!}</p>
              @else
              Description:

              <p> {!! $PaidDetails->description_en !!}</p>
              @endif




               @if($PaidDetails->attachments)
                ...
                <div class="attachments">
                     ...
                     @foreach ($PaidDetails->attachments as $attachment )
                     ...
                     <div class="attach1">
                        ...
                        <div class="openbtn">
                            ...
                            href="{{route('user.Download.various.attachment' ,[ $PaidDetails->id , $attachment->id ])}}"
            ...
            <div class="comments">
                <h6>{{ count( $PaidDetails->reviews) }} {{ trans('article_trans.reviews') }}</h6>
                <div class="other-comments">
                    @foreach( $PaidDetails->reviews as $review)
                  ...
                  <form action="{{ route('user.various.review' ,$PaidDetails->id ) }}" method="post">
                    @csrf
                         @method('post')
      ...
      <div class="sideContent">
          ...
          <div class="more-videos notpaid">
            ...
            <div class="onevideo">
                <a  href="{{ route('user.paidVideoShow' ,$recentVideo->id  ) }}">
                    <img   width="200px" src="{{ asset( $recentVideo->poster) }}" ></img>

                </a>
                <div class="sidedetails">
                    @if(App::getLocale() == 'ar')
                    <a href="{{ route('user.paidVideoShow' ,$recentVideo->id  ) }}"><h5>{{ $recentVideo->title_ar }}</h5></a>

                  @else
                  <a href="{{  route('user.paidVideoShow' ,$recentVideo->id  ) }}"><h5>{{ $recentVideo->title_en }}</h5></a>
...
// Eend Product View with Modal
 // Start Add To Cart Product
    ...
    function likedVideos(){
        var id ="{{ $PaidDetails->id }}"

        $.ajax({
            type: "post",
            dataType: 'json',

            url: "{{ route('user.liked.videos', $PaidDetails->id ) }}",
...
function Share(share)
{
    ...
    {
        case "facebook":
            window.open("https://www.facebook.com/sharer/sharer.php?u={{route('user.paidVideoShow',$PaidDetails->id)}}&text={{$PaidDetails->title_en}}")
            break;
            case "twitter":
            window.open("https://twitter.com/intent/tweet?url={{route('user.paidVideoShow',$PaidDetails->id)}}")
            break;
            case "whatsapp":
            window.open("https://wa.me/?text={{route('user.paidVideoShow',$PaidDetails->id)}}")
