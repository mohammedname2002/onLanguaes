                        ...
                        <h4 class="card-title">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-6" style="text-align: left">
                                    <a href="{{route('admin.role.create')}}" class="btn btn-primary">إنشاء</a>
                                </div>
                            </div>
                        </h4>
                            ...
                            <table style="border-collapse: collapse; border-spacing: 0px; width: 100%;" id="datatable" class="table table-bordered dt-responsive nowrap dataTable no-footer dtr-inline">
                                ...
                                <tbody>
                                    ...
                                    <tr data-id="1" style="cursor: pointer;">
                                        <td data-field="id" style="width: 80px">{{ $roles->firstItem() + $loop->index }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $role->name }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $role->permissions_count }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $role->users_count }}</td>
                                        <td data-field="age" style="width: 182.783px;">{{ $role->created_at->format('y-m-d') }}</td>
                                        <td style="width: 200px;" class="d-flex justify-content-center">
                                            <a class="btn btn-outline-secondary btn-sm edit" href="{{route('admin.role.edit',$role->id)}}" title="تعديل">
                                                <i class="fas fa-pencil-alt" title="تعديل"></i>
                                            </a>
                                             <form action="{{route('admin.role.delete',$role->id)}}" id="deleteform{{$role->id}}" method="POST" style="margin-right:4px;">
                                                @csrf
                                                <button type="submit" onclick="JSconfirm(event,{{$role->id}})" class="btn btn-outline-secondary btn-sm edit">
