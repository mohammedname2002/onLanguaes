                        ...
                        <h4 class="card-title">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-6" style="text-align: left">
                                    <a href="{{route('admin.student.create')}}" class="btn btn-primary">إنشاء</a>
                                </div>
                            </div>
                        </h4>
                                    ...
                                    <div class="col-sm-12 col-md-6" >
                                        <div id="datatable_filter" class="dataTables_filter" style="text-align: right !important">
                                            <label>بحث:<input name="search" value="{{request('search')}}" type="search" placeholder="بحث عن..." class="form-control form-control-sm" placeholder="" aria-controls="datatable"></label>
                                             <span style="margin-right: 8px;">عدد الطلاب:{{$info->count}}</span>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-6">
                            ...
                            <table style="border-collapse: collapse; border-spacing: 0px; width: 100%;" id="datatable" class="table table-bordered dt-responsive nowrap dataTable no-footer dtr-inline">
                                ...
                                <tbody>
                                    ...
                                    <tr data-id="1" style="cursor: pointer;">
                                        <td data-field="id" style="width: 80px">{{ $students->firstItem() + $loop->index }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $student->name }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $student->email }}</td>
                                        <td data-field="name" style="width: 445.65px;">{{ $student->courses }}</td>
                                        <td data-field="age" style="width: 182.783px;">{{ $student->created_at->format('y-m-d') }}</td>
                                        <td style="width: 200px;" class="d-flex justify-content-center">
                                            @if ($admin->can('تعديل طالب') || $admin->can('جميع الصلاحيات super_admin')  )

                                            <a class="btn btn-outline-secondary btn-sm edit" href="{{route('admin.student.edit',$student->id)}}" title="تعديل">
                                                <i class="fas fa-pencil-alt" title="تعديل"></i>
                                            </a>
                                            @endif
                                            @if ($admin->can('حذف طالب') || $admin->can('جميع الصلاحيات super_admin')  )

                                             <form action="{{route('admin.student.delete',$student->id)}}" id="deleteform{{$student->id}}" method="POST" style="margin-right:4px;">
                                                @csrf
                                                <button type="submit" onclick="JSconfirm(event,{{$student->id}})" class="btn btn-outline-secondary btn-sm edit">
